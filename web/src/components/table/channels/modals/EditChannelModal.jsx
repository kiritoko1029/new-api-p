...
import ChannelSessionPanel from './ChannelSessionPanel';
...
const EditChannelModal = (props) => {
  ...
  const handleChannelSettingsChange = (key, value) => {
    const nextChannelSettings = { ...channelSettings, [key]: value };
    const settingsJson = JSON.stringify(nextChannelSettings);
    ...
    // 同步更新inputs状态
    setInputs((prev) => ({ ...prev, [key]: value, setting: settingsJson }));
  };

  const handleChannelOtherSettingsChange = (key, value) => {
    let settings = {};
    const currentSettings =
      formApiRef.current?.getValue('settings') ?? inputs.settings;
    if (currentSettings) {
      try {
    ...
    if (success) {
      ...
      if (data.settings) {
        try {
          ...
          const claudeSessionFormValues =
            getClaudeSessionFormValues(parsedSettings);
          ...
          data.claude_max_sessions =
            claudeSessionFormValues.claude_max_sessions;
          data.claude_session_ttl_minutes =
            claudeSessionFormValues.claude_session_ttl_minutes;
          ...
          data.claude_max_sessions = 0;
          data.claude_session_ttl_minutes = 30;
  ...
  useEffect(() => {
    ...
    if (!isEdit) {
      ...
      const localModels = getChannelModels(originInputs.type);
      const initialInputs = { ...originInputs, models: localModels };
      setInputs(initialInputs);
      if (formApiRef.current) {
        formApiRef.current.setValues(initialInputs);
      }
      setBasicModels(localModels);
    }
  ...
  useEffect(() => {
    ...
    if (props.visible) {
      if (isEdit) {
        ...
        const initialInputs = getInitValues();
        const localModels = getChannelModels(initialInputs.type);
        const nextInputs = { ...initialInputs, models: localModels };
  ...
  useEffect(() => {
    if (!props.visible || !advancedSettingsOpen || !formApiRef.current) {
      return;
    }
    formApiRef.current.setValues(inputs);
  }, [advancedSettingsOpen, inputs, props.visible]);
  ...
  const submit = async () => {
    ...
    let localInputs = {
      ...formValues,
      settings: formValues.settings ?? inputs.settings,
      setting: formValues.setting ?? inputs.setting,
    };
    ...
    if (localInputs.type === 1 || localInputs.type === 14) {
      ...
      if (localInputs.type === 14) {
        ...
        settings = applyClaudeSessionSettings(settings, localInputs);
      }
    }
    ...
    delete localInputs.claude_max_sessions;
    delete localInputs.claude_session_ttl_minutes;
                ...
                <div className='pt-3'>
                  ...
                  {([14, 58, 59, 60].includes(inputs.type)) && (
                    <>
                      ...
                      />
                      <Form.InputNumber
                        field='claude_max_sessions'
                        label={t('最大会话数')}
                        placeholder={t('0 = 不限制')}
                        initValue={inputs.claude_max_sessions}
                        min={0}
                        max={1000}
                        step={1}
                        onNumberChange={(value) =>
                        ...
                        extraText={t(
                          '限制该渠道同时承载的最大会话数。当活跃会话数达到上限时，新会话将被路由到其他渠道。0 或留空表示不限制',
                        )}
                      />
                      <Form.InputNumber
                        field='claude_session_ttl_minutes'
                        label={t('会话超时（分钟）')}
                        placeholder={t('默认 30')}
                        initValue={inputs.claude_session_ttl_minutes}
                        min={1}
                        max={1440}
                        step={5}
                        onNumberChange={(value) =>
